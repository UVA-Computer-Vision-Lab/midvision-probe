...
dataset_path = "/p/openvocabdustr/probing_midlevel_vision/code/probing-mid-level-vision/data/nyu_geonet_hf_datasets_v2"
parquet_save_path = "/p/openvocabdustr/probing_midlevel_vision/code/probing-mid-level-vision/data/nyu_geonet_hf_datasets_parquet"
...
def process_file(file_name, dataset_path):
    """
    Process a single file, loading image, depth, normal, and segmentation.
    ...
    try:
        ...
        # Convert id2label to a JSON string
        id2label_str = json.dumps(id2label)

        # Create a record
        return {
            "image": image.flatten().tolist(),  # Convert 3D image to 1D list
            "depth": depth.flatten().tolist(),  # Convert depth to 1D list
            "normal": normal.flatten().tolist(),  # Convert normal to 1D list
            "segmentation": panoptic_map.flatten().tolist(),  # Convert segmentation to 1D list
            "id2label": id2label_str,  # Include id2label mapping
        }
...
def save_to_parquet(records, chunk_id, parquet_save_path):
    ...
    if valid_records:
        ...
        # Convert dict of arrays into a dict of lists for each column in the schema
        columns = {
...
def process_in_parallel(
    dataset_path, parquet_save_path, num_workers=4, chunk_size=1000
):
    """
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        with tqdm(total=total_files, desc="Processing files") as pbar:
            for file_name in image_files:
                future = executor.submit(process_file, file_name, dataset_path)
...
if __name__ == "__main__":
    ...
    process_in_parallel(dataset_path, parquet_save_path, num_workers=20)
