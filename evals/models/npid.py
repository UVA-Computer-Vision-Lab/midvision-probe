...
def load_model(arch: str, **kwargs):
    ...
    ckpt = prepare_state_dict(
        ckpt, remove_prefix="_feature_blocks.", delete_prefixes=["fc"]
    )
...
class NPID(nn.Module):
    def __init__(
        ...
        self.output = output
        self.add_norm = add_norm  # Store the flag for batch normalization
        self.return_layers = (
        ...
        self.checkpoint_name = f"$npid$_{arch}_{output}_{self.return_layers}"
        self.patch_size = 0
