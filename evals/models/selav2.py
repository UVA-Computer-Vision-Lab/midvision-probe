...
def load_model(arch: str, **kwargs):
    ...
    ckpt = prepare_state_dict(
        ...
        delete_prefixes=["projection_head.", "prototypes."],
    )
    model.load_state_dict(ckpt)
    return model


class SELAV2(nn.Module):
    def __init__(
        self,
        arch="resnet50",
        return_layers=None,
        ...
        self.checkpoint_name = f"$sela_v2$_{arch}_{output}_{self.return_layers}"
        self.patch_size = 0
        ...
        # Define BatchNorm2d layers for each return layer if add_norm is enabled
