...
checkpoints = {
    "resnet50": {
        "url": "https://huggingface.co/xinlongwang/DenseCL/resolve/main/densecl_r50_imagenet_200ep.pth?download=true",
...
class DENSECL(nn.Module):
    def __init__(
        ...
        self.checkpoint_name = f"$densecl$_{arch}_{output}_{self.return_layers}"
        self.patch_size = 0
        ...
        # Define BatchNorm2d layers for each layer in the architecture
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x)
            # If add_norm is True, apply BatchNorm2d after each layer's output
